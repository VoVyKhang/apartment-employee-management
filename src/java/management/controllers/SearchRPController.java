    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String keywordname=request.getParameter("txtSearchName");
                    ArrayList<RewardPenaltyDTO> listrp ;
                    if(keywordidemp==null || keywordname==null)
                        listrp = RewardPenaltyDAO.listrp("");
                    else if(keywordidemp != null && keywordname==null){
                         listrp = RewardPenaltyDAO.listrp(keywordidemp);
                    }else if(keywordname != null && keywordidemp==null){
                        listrp = RewardPenaltyDAO.listrpdep(keywordname); 
                    } else 
                        listrp = RewardPenaltyDAO.listrpdepemp(keywordname, keywordidemp);
            request.setAttribute("listrp", listrp);
            request.getRequestDispatcher("listRP.jsp").forward(request, response); 
        }
    }
